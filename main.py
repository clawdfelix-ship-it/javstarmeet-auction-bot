...
async def handle_numpad_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "set":
        ...
        # Process bid
        await process_bid(user, price, query=None, bot=context.bot)
...
def generate_bid_keyboard(current_price):
    # 固定 6 個面額：+10, +20, +30, +50, +100, +500
    increments = [10, 20, 30, 50, 100, 500]
        
    buttons = []
    row = []
    for inc in increments:
        new_price = current_price + inc
        # Check if new_price exceeds BIN price (if set)
        bin_price = current_auction.get("bin_price", 0)
        if bin_price > 0 and new_price >= bin_price:
             continue # Don't show bid buttons higher than BIN
        
        row.append(InlineKeyboardButton(f"${new_price}", callback_data=f"bid_{new_price}"))
...
async def handle_private_bid_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Process the bid
    await process_bid(user, price, query=None)
...
async def handle_bid_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            await context.bot.send_message(
             ...
             try:
                ...
                if not bot_username:
                    ...
                    bot_username = me.username
                url = f"https://t.me/{bot_username}?start=bid"
                await query.answer("⚠️ 無法在群組發言，請點此私訊出價", url=url)
        return
    ...
    if data.startswith("bid_bin_"):
        ...
        if current_auction["bin_price"] > 0 and bin_price == current_auction["bin_price"]:
             await process_bid(user, bin_price, query, context.bot)
        else:
             await query.answer("❌ 一口價無效或已變更", show_alert=True)
        return

    # Handle normal increments
    # Format is bid_{price} (absolute price) NOT increment
    # Wait, let's check generate_bid_keyboard
    # It generates callback_data=f"bid_{new_price}" which is current + inc
    # So data.split("_")[1] is the target price.
    
    target_price = int(data.split("_")[1])
    
    await process_bid(user, target_price, query, context.bot)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        ...
        "2️⃣ <b>出價方式</b>：\n"
        "   • 點擊拍賣訊息下方的快捷按鈕 (例如 +$10)。\n"
        "   • <b>私聊出價</b>：若群組禁言中，可直接私訊 Bot 輸入數字出價。\n"
        "3️⃣ <b>拍賣時限</b>：\n"
        "   • 每場拍賣預設 25 秒倒數。\n"
...
async def handle_text_bid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if bid_price <= current_auction["current_price"]:
        if is_valid_reply:
            await msg.reply_text(f"❌ 出價必須高於當前價格 (${current_auction['current_price']})")
        return

    await process_bid(user, bid_price, None, context.bot)
    try:
        await msg.delete()
    except:
        pass
...
async def process_bid(user, price, query=None, bot=None):
    ...
    if bin_price > 0 and price >= bin_price:
        ...
        if target_bot:
             ...
             # Should not happen if bot passed correctly, but as fallback
             # We need a way to end auction if no bot instance.
             # However, process_bid usually called with bot or query.
             pass
...
async def handle_webapp_bid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Process bid directly
    if price <= current_auction["current_price"]:
        # WebApp doesn't show alert easily unless we reply
        # But we can just ignore or send a fleeting message?
        # Better to just process_bid which handles logic
        pass
